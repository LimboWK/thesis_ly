...
Understanding spatial openness in residential housing is important for enhancing living quality and 
guiding architectural design. Traditional indicators such as floor area and room count fail to 
capture the nuanced spatial qualities present in interior imagery and floor plans. While existing 
research has explored aspects of spatial openness, a comprehensive evaluation framework is lacking. 
This study proposes a multifaceted evaluation method of spatial openness in rental housing 
by leveraging large-scale data from Tokyo's 23 wards.

The study constructs an analytical method that quantitatively extracts housing openness from 
both 2D and 3D information. Based on a large-scale rental housing dataset from Tokyo's 23 wards, 
it investigates how openness features vary across geographic regions and construction decades, 
identifying spatial and temporal patterns. Furthermore, it explores correlations between spatial 
openness, rental cost, and subjective impression scores. In conclusion, this study proposes a 
comprehensive framework for evaluating spatial openness, making full use of large-scale rental housing 
data from Tokyo's 23 wards.
...
Our dataset comprises rental properties from Tokyo's 23 wards (1960-present), filtered for image quality and data completeness, 
as shown in Figure \ref{fig:data_selection}.
\begin{center}
    \includegraphics[width=0.85\columnwidth]{plots/overview.png}
...
We examined the distribution of mean visibility values across construction dates in decades, as shown in 
Figure \ref{fig:temporal_analysis}.

The analysis shows mean visibility values contain large outliers but remain stable across decades, likely due to large-area properties. 
...
\begin{center}
    ...
    \captionof{figure}{Geographic distribution analysis across Tokyo's 23 special wards: impression scores (top) and mean visibility values (bottom).}
...
using mean visibility values, ceiling, wall, floor, and window visual fractions as input features. The analysis reveals 
...
min\_all & 1.35 & Low \\
max\_all & 28.16 & High \\
mean\_all & 8.34 & Moderate \\
std\_all & 5.58 & Moderate \\
min\_lv & 3.40 & Low \\
max\_lv & 37.94 & High \\
std\_lv & 3.63 & Low \\
median\_lv & 18.29 & High \\
window & 1.06 & Low \\
floor & 1.21 & Low \\
ceiling & 1.13 & Low \\
wall & 1.23 & Low \\
...
Feature & VIF & Status \\
\hline
min\_value\_raw\_all & 1.33 & Low \\
max\_value\_raw\_all & 9.24 & Moderate \\
mean\_value\_raw\_all & 5.42 & Moderate \\
std\_value\_raw\_all & 4.09 & Low \\
min\_value\_raw\_lv & 2.86 & Low \\
std\_value\_raw\_lv & 2.92 & Low \\
ceiling & 1.03 & Low \\
\hline
\end{tabular}
\captionof{table}{Variance Inflation Factor (VIF) for multicollinearity assessment (after feature selection), VIF > 10: High,
VIF > 5: Moderate,
VIF < 5: Low}
\label{tab:vif_analysis_updated}
\end{center}
...
\hline
\end{tabular}
\captionof{table}{Feature coefficients for rent prediction (normalized features), their t-stats and p-values in the test againest the impact towards the rent price. Confidence level set to 95\%.}
...
Feature & Coef. & Std & t-stat & p-value & Sig. \\
 & ($\times 10^3$) & ($\times 10^2$) & & & \\
\hline
min\_all & -2.15 & 5.95 & -3.61 & 0.00 & Y \\
max\_all & 25.48 & 15.67 & 16.27 & 0.00 & Y \\
mean\_all & -16.63 & 12.00 & -13.86 & 0.00 & Y \\
std\_all & 12.13 & 10.42 & 11.64 & 0.00 & Y \\
min\_lv & -3.37 & 8.72 & -3.86 & 0.00 & Y \\
std\_lv & -6.80 & 8.81 & -7.72 & 0.00 & Y \\
ceiling & 8.30 & 5.23 & 15.86 & 0.00 & Y \\
\hline
\end{tabular}
\captionof{table}{Feature coefficients (after feature selection) for rent\_plus\_management\_fee prediction (normalized features), their t-stats and p-values in the test against the impact towards the rent price. Confidence level set to 95\%.}
\label{tab:regression_coefficients_plus_fee}
\end{center}
...
\hline
min\_all & Minimum visibility value (whole property) \\
max\_all & Maximum visibility value (whole property) \\
mean\_all & Mean visibility value (whole property) \\
std\_all & Standard deviation of visibility values (whole property) \\
min\_lv & Minimum visibility value (living room) \\
max\_lv & Maximum visibility value (living room) \\
std\_lv & Standard deviation of visibility values (living room) \\
median\_lv & Median visibility value (living room) \\
window & Window fraction \\
floor & Floor fraction \\
ceiling & Ceiling fraction \\
wall & Wall fraction \\
\hline
\end{tabular}
\captionof{table}{Feature abbreviations and descriptions.}
\label{tab:feature_abbreviations}
\end{center}
