...
openness from interior images and floor plan data. The openness features extractions include the windows ratio, ceilings ratio, floors ratio, 
...
These features are used in subsequent temporal, spatial, and correlation analyses.

\section{Overview of the LIFULL Real Estate Dataset}

Our dataset comprises 4004 rental properties from Tokyo's 23 wards (1960-present), filtered for image availability 
and completeness. The LIFULL dataset represents one of Japan's largest real estate databases, providing comprehensive 
information about rental properties including interior images, floor plan data, and property specifications.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{chapters/plots/data_filtering.png}
...
The following sections detail the extraction process of the openness features that have been already mentioned
in the data filtering and cleaning workflow. 

\section{Openness Features Extractions}
...
Corresponding to the step 4 in the above section, the VGA value quantifies the degree of spatial integration and visual connectivity within a given floor plan layout. For a discretized spatial grid with $N$ points, the visibility graph analysis computes connectivity measures based on line-of-sight relationships between grid points.

Mathematically, the VGA value at each point is defined as:
\begin{equation}
...
\begin{itemize}
    ...
    \end{cases}
    \end{equation}
    \item $N$ is the total number of grid points in the open space
\end{itemize}

The visibility determination between two points follows the isovist principle, where point $j$ is considered visible from point $i$ if the straight line connecting them does not intersect any wall boundaries in the floor plan.

This computation generates a VGA heatmap $\mathbf{S} = \{S(1), S(2), \ldots, S(N)\}$ representing spatial connectivity throughout the floor plan. From this heatmap, summary statistics including mean ($\mu_S$), standard deviation ($\sigma_S$), and percentiles are extracted as quantitative features characterizing spatial openness.
...
\begin{enumerate}
    \item \textbf{Semantic Segmentation}: DeepLab-V3+ model \cite{liu2024image} fine-tuned to classify walls, bedrooms, and living rooms
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{chapters/plots/vga_workflow.png}
...
\subsection{Architectural Element Ratios}

Beyond VGA analysis, spatial openness is quantified through the analysis of key architectural elements extracted from 
...
The semantic segmentation employs a Mask2Former pre-trained model \cite{cheng2022masked} optimized for architectural element recognition.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{chapters/plots/inter_semseg.png}
